...
class MuseumsController {
    ...
    index = async (req, res, next) => {
        try {
            ...
            if (limit && skip) {
                ...
                posts = await Museums.paginate({}, options);
            } else {
                posts = await Museums.find().populate('category').sort({ created_at: -1 }).exec();
            }
    ...
    show = async (req, res, next) => {
        try {
            const { id } = req.params;
            const item = await Museums.findById(id).exec();
    ...
    edit = async (req, res, next) => {
        ...
        try {
            const post = await Museums.findById(id).exec();
    ...
    update = async (req, res, next) => {
        ...
        try {
            ...
            const post = await Museums.findOneAndUpdate({ _id: id }, postUpdate, { new: true }).exec();
    ...
    destroy = async (req, res, next) => {
        ...
        try {
            ...
            let { mode } = req.query;
            if (mode) {
                post = await Museums.findByIdAndUpdate({ _id: id }, { deleted_at: (mode === 'softdelete' ? Date.now() : null) }, { new: true });
            } else {
                mode = 'delete';
                post = await Museums.findOneAndRemove({ _id: id });
            }
