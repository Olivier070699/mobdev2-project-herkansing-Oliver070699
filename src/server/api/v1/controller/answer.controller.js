...
class AnswerController {
    ...
    index = async (req, res, next) => {
        try {
            ...
            if (limit && skip) {
                ...
                posts = await Answer.paginate({}, options);
            } else {
                posts = await Answer.find().populate('category').sort({ created_at: -1 }).exec();
            }
    ...
    show = async (req, res, next) => {
        try {
            const { id } = req.params;
            const item = await Answer.findById(id).exec();
    ...
    edit = async (req, res, next) => {
        ...
        try {
            const post = await Answer.findById(id).exec();
    ...
    update = async (req, res, next) => {
        ...
        try {
            ...
            const post = await Answer.findOneAndUpdate({ _id: id }, postUpdate, { new: true }).exec();
    ...
    destroy = async (req, res, next) => {
        ...
        try {
            ...
            let { mode } = req.query;
            if (mode) {
                post = await Answer.findByIdAndUpdate({ _id: id }, { deleted_at: (mode === 'softdelete' ? Date.now() : null) }, { new: true });
            } else {
                mode = 'delete';
                post = await Answer.findOneAndRemove({ _id: id });
            }
